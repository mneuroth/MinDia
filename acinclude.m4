...
AC_MSG_RESULT($enable_svnsupport)
 ...
 AC_MSG_CHECKING(for Subversion svn-config)
	...
	AM_CONDITIONAL(include_svn, test -n "$SVN")
])
...
[
  AC_MSG_CHECKING(for Berkeley DB >= 3)
  ...
  if test "$ac_cv_bdb" = "no"; then
    AC_MSG_ERROR(no - please install Berkeley DB >= 3)
  else
    if test "$ac_cv_bdb" = "yes_3or40" ; then
      bdb_open_hack=""
      AC_MSG_RESULT([yes - <=4.0])
    else
      bdb_open_hack=",0"
  ...
  AC_DEFINE_UNQUOTED(BDB_OPEN_HACK, $bdb_open_hack, [account for Berkeley DB API different from v4.1 onwards])
])
...
[
  ...
  if test "$ac_cv_mdi_setup" = "yes"; then
    ...
    LIB_KDEVMDI='$(top_builddir)/lib/qextmdi/libkdevelopqextmdi.la'
    QEXTMDIR="qextmdi"
    KDEVMDI_INCLUDES='-I$(top_srcdir)/lib/qextmdi'
    AC_MSG_RESULT(no)
  fi
 
  AC_SUBST(LIB_KDEVMDI)
  AC_SUBST(KDEVMDI_INCLUDES)
  AC_SUBST(QEXTMDIR)
  AM_CONDITIONAL(include_qextmdi, test -n "$QEXTMDIR")
])
...
dnl  stolen from licq's qt-plugin, thank you guys ;)
dnl  **********

AC_DEFUN(KDEV_CHECK_QEDITOR,
[
  AC_MSG_CHECKING(for Qt >= 3.0.5)
...
AC_SUBST(QEDITOR)
AM_CONDITIONAL(include_qeditor, test -n "$QEDITOR")
])
